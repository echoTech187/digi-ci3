...
class QRISDynamic extends CI_Model
{
    ...
    public function get_summary($search_name = null, $search_date = null, $search_date_to = null, $search_transid = null, $search_status = null, $search_reff = null)
    {
        ...
        if ($is_history_included) {
            ...
            $this->db->where('transaction_type', 'QRIS_DYNAMIC');
        ...
        if ($is_today_included) {
            ...
            $this->db->where('cdq.c_datetimeRequest >=', $today . ' 00:00:00');
            $this->db->where('cdq.c_datetimeRequest <=', $today . ' 23:59:59');

            $this->_apply_filters($search_name, $today, $search_transid, $search_status, $search_reff, $today);
            
            $live = $this->db->get()->row();
            $total_qty += $live->qty ?: 0;
            $total_amount += $live->total_amount ?: 0;
        }
