...
$(document).ready(function() {
    ...
    $('input[name="update_type"]').on('change', function() {
        if ($(this).val() === 'merchant') {
            $('#merchantSelectGroup').slideDown(300);
            $('#global_merchant').prop('required', true);
        } else {
            $('#merchantSelectGroup').slideUp(300);
            $('#global_merchant').prop('required', false).val(null).trigger('change');
        }
    });

    // AJAX: Sync Channel IDs for Current Filter
    function updateGlobalCurrentChannelIds() {
        const group = $('#global_current_group').val();
        const ext = $('#global_current_external').val();
        const $chan = $('#global_current_channel');

        if (group && ext) {
            $chan.prop('disabled', true).html('<option value="">Loading...</option>');
            $.ajax({
                url: '<?= base_url("admin/getCashoutChannelGroups"); ?>',
                type: 'POST',
                data: { 
                    c_cashoutChannelGroup: group, 
                    c_externalIdDefault: ext,
                    [csrfName]: csrfHash 
                },
                dataType: 'json',
                success: function(res) {
                    let html = '<option value="">All Channel IDs</option>';
                    if (res && res.length > 0) {
                        res.forEach(item => {
                            html += `<option value="${item.id}">${item.id}</option>`;
                        });
                    }
                    $chan.html(html).prop('disabled', false).trigger('change');
                },
                error: function() {
                    $chan.html('<option value="">All Channel IDs</option>').prop('disabled', false);
                }
            });
        } else {
            $chan.html('<option value="">All Channel IDs</option>').prop('disabled', false);
    ...
    $('#global_current_external, #global_current_group').on('change', updateGlobalCurrentChannelIds);

    // AJAX: Sync Channel IDs for Target Configuration
    function updateGlobalNewChannelIds() {
        const group = $('#global_new_group').val();
        const ext = $('#global_new_external').val();
        const $chan = $('#global_new_channel');
        ...
        if (group && ext) {
            ...
            $.ajax({
                ...
                dataType: 'json',
                success: function(res) {
                    let html = '<option value="">Don\'t Update (Keep Original)</option>';
                    if (res && res.length > 0) {
                        res.forEach(item => {
                            html += `<option value="${item.id}">${item.id}</option>`;
                        });
                ...
                error: function() {
                    $chan.html('<option value="">Don\'t Update (Keep Original)</option>').prop('disabled', false);
                }
            });
        } else {
            $chan.html('<option value="">Don\'t Update (Keep Original)</option>').prop('disabled', false);
    ...
    $('#global_new_external, #global_new_group').on('change', updateGlobalNewChannelIds);
