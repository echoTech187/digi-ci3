/**
 * Cashin Channel Master Configuration
 */
$(document).ready(function() {
    ...
    const listUrl = window.BASE_URL + 'channel/cashin';
    const deleteBaseUrl = window.BASE_URL + 'channel/cashin/delete/';
                ...
                return `
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right shadow border-0 py-2">
                            <li><button type="button" class="dropdown-item edit-cashin" data-toggle="modal" data-target="#editChanelModal" data-id="${row.id}" data-channelid="${row.ref_cashinChannelId}" data-group="${row.c_channelGroup}" data-desc="${row.c_description || ''}" data-ext="${row.c_externalIdDefault || ''}" data-feetype="${row.c_feeType || ''}" data-fee="${row.c_fee || 0}" data-min="${row.c_amountMin || 10000}" data-max="${row.c_amountMax || 10000000}" data-interval="${row.c_settlementInterval || 1}"><i class="fas fa-edit text-primary mr-2"></i> Edit Channel</button></li>
                            <li><button type="button" class="dropdown-item delete-cashin text-danger" data-id="${row.id}"><i class="fas fa-trash-alt mr-2"></i> Delete Channel</button></li>
                        </ul>
                    </div>
                `;
            }
    ...
    $(document).on('click', '.edit-cashin', function() {
        ...
        else if (ft === 'percentage' || ft === 'percetange') $('#edit_feetype').val('Percetange');
