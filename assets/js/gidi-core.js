$(document).ready(function () {
	...
	var $flyoutBackdrop = $('<div class="sb-flyout-backdrop"></div>').appendTo(
		"body",
	);
	...
	$(document).on("click", ".dt-more-filters-btn", function () {
		var $panel = $(this)
			.closest(".dt-more-filters-wrapper")
			.find(".dt-more-panel");
		$panel
			.find("select")
	...
	function initMobileTableLabels() {
		if (window.innerWidth <= 768) {
			...
			$(".btn-dt-secondary, .btn-dt-primary")
				.has(".fa-sync-alt, .fa-sync")
				.each(function () {
					var textNodes = $(this)
						.contents()
						.filter(function () {
					...
					if (textNodes.length > 0) {
						...
						$(this).append(
							'<span class="btn-text-mobile-hide">' + text + "</span>",
						);
	...
	function portFiltersToDrawer() {
		if (window.innerWidth <= 768) {
			...
			$(".dt-toolbar").each(function () {
				...
				if ($filtersToMigrate.length > 0 && $target.length > 0) {
					...
					$filtersToMigrate.each(function () {
						...
						if ($subBtn.length > 0) {
							...
							if ($footer.find('button[type="submit"]').length === 0) {
								...
								var $applyBtn = $subBtn
									.clone()
									.appendTo($footer)
									.addClass("btn-block btn-dt-primary")
									.html('<i class="fas fa-check mr-1"></i> APPLY FILTER');
								...
								var $cancelBtn = $(
									'<button type="button" class="btn-dt-cancel" id="btnCloseFilters">CANCEL</button>',
								).appendTo($footer);
							...
							$toolbar.on(
								"click",
								'.dt-more-panel-footer button[type="submit"], .dt-more-panel button[type="submit"]',
								function () {
									$toolbar.find(".dt-more-filters-btn").removeClass("dt-open");
									$toolbar.find(".dt-more-panel").removeClass("dt-panel-open");
								},
						...
						$group
							.find(".dt-filter-label")
							.addClass("dt-more-label")
							.removeClass("dt-filter-label");
						$group
							.find(".dt-filter-chip")
							.addClass("dt-more-field-container")
							.removeClass("dt-filter-chip");
						...
						if ($target.hasClass("dt-panel-open")) {
							...
							$target
								...
								.filter(function () {
									...
									return (
										text.includes("reset") ||
										$(this).find("i.fa-undo").length > 0
									);
								})
								.not(".btn-dt-cancel, #btnCloseFilters")
								.hide();
						} else {
			...
			$(document).on(
				"click",
				".dt-panel-overlay, .btn-dt-cancel, #btnCloseFilters, .dt-more-panel-footer button.btn-dt-secondary",
				function () {
					$(".dt-more-panel").removeClass("dt-panel-open");
					$(
						".dt-more-filters-btn, #btnToggleFilters, #vaMoreFiltersBtn",
					).removeClass("dt-open");
				},
			);
			...
			$(
				".dt-more-panel .dt-mobile-reset-icon, .dt-more-panel .btn-dt-secondary",
			)
				.filter(function () {
					return (
						$(this).text().trim().toLowerCase().includes("reset") ||
						$(this).find("i.fa-undo").length > 0
					);
				})
...
$(document).ready(function () {
		...
		if (!isDbOffline) {
			...
			if ($("#maintenance_label").length) {
				...
				$("#maintenance_dot")
					...
					.addClass("bg-danger");
				$("#maintenance_ping")
					.removeClass("bg-success bg-secondary bg-warning animate-ping")
					.addClass("bg-danger");
			}
			...
			if (!offlineAlertOpen) {
				...
				Swal.fire({
					...
					html:
						'<div class="text-left"><p class="mb-2">We have detected a temporary loss of database connectivity. To prevent data corruption, form submissions have been paused.</p><p class="text-xs text-muted mb-0">Diagnostic: ' +
						msg +
						"</p></div>",
					icon: "warning",
	...
	function handleDbOnline() {
		...
		if (typeof loadMetadata === "function") {
			...
			$("#maintenance_dot")
				.removeClass("bg-danger bg-secondary bg-warning")
				.addClass("bg-success");
			$("#maintenance_ping")
				.removeClass("bg-danger bg-secondary bg-warning")
				.addClass("bg-success animate-ping");
		}
