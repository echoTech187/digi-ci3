/**
 * Merchant Cashin Fee Settings Table & Modal Controls
 */
$(document).ready(function() {
    ...
    const listUrl = window.BASE_URL + 'merchant/setting-cashin-fee/' + merchantId;
    ...
    var table = initServerDataTable("#cashinFeeTable", listUrl, [
            ...
            render: function(data, type, row) {
                return `
                    <div class="d-flex justify-content-center gap-1">
                        ...
                        <button type="button" class="btn-dt-action-icon btn-dt-action-delete delete-fee-btn" data-href="${window.BASE_URL}merchant/setting-cashin-fee/delete/${merchantId}/${row.id}" title="Delete Fee"><i class="fas fa-trash fa-xs"></i></button>
                    </div>
                `;
            }
        }
    ...
    $('#c_cashinChannelGroup, #c_externalIdDefault').on('change', function() {
        ...
        if (group && ext) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#ref_cashinChannelId').html(opts).prop('disabled', false);
                }
            });
        }
    });
    ...
    $(document).on('click', '.edit-fee-btn', function() {
        ...
        $('#feeModalTitle').text('Edit Cashin Fee Setting');
        $('#feeForm').attr('action', window.BASE_URL + 'merchant/setting-cashin-fee/update/' + row.id);
        ...
        setTimeout(() => {
            $('#ref_cashinChannelId').val(row.ref_cashinChannelId).trigger('change');
        }, 300);
    ...
    $('.add-btn').on('click', function() {
        $('#feeModalTitle').text('Add Cashin Fee Setting');
        $('#feeForm').attr('action', window.BASE_URL + 'merchant/setting-cashin-fee/create');
        $('#feeForm')[0].reset();
